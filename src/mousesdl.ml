...
(** Handles an SDL mouse button event.
    Returns the updated mouse state and a list of unified events. *)
let handle_mouse_button_event (event: Sdl.event) (m: Mouse.t) : Mouse.t * Event.t list =
  ...
  if pressed then
    (m, [MouseButtonDown (button, (x / Mouse.get_scale m, y / Mouse.get_scale m))])
  else
    (m, [MouseButtonUp (button, (x / Mouse.get_scale m, y / Mouse.get_scale m))])

(** Handles an SDL mouse motion event.
    Returns the updated mouse state and a list of unified events. *)
let handle_mouse_motion_event (event: Sdl.event) (m: Mouse.t) : Mouse.t * Event.t list =
  ...
  let pressed_buttons = List.filter (fun b -> Mouse.is_button_pressed m b) buttons in
  if pressed_buttons = [] then
    (m, [MouseMotion (x / Mouse.get_scale m, y / Mouse.get_scale m)])
  else
    let drag_events = List.map (fun b -> MouseDrag (b, (x / Mouse.get_scale m, y / Mouse.get_scale m))) pressed_buttons in
    (m, drag_events)

(** Handles an SDL mouse wheel event.
    Returns the updated mouse state and a list containing the wheel event. *)
let handle_mouse_wheel_event (event: Sdl.event) (m: Mouse.t) : Mouse.t * Event.t list =
  let y = Sdl.Event.get event Sdl.Event.mouse_wheel_y in
  (m, [MouseWheel y])

(** Process any SDL mouse-related event.
    Returns the updated mouse state and any unified events. *)
let handle_event (event: Sdl.event) (m: Mouse.t) : Mouse.t * Event.t list =
  ...
  if event_type = Sdl.Event.mouse_button_down || event_type = Sdl.Event.mouse_button_up then
    handle_mouse_button_event event m
