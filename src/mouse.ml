...
let create scale =
  ...
  else { position = (0, 0); buttons = [(Left, false); (Middle, false); (Right, false)]; scale }

let update_position t (x, y) =
  { t with position = (x / t.scale, y / t.scale) }

let update_button t button pressed =
  { t with buttons = List.map (fun (b, state) -> if b = button then (b, pressed) else (b, state)) t.buttons }
