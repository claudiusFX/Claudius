...
let to_array (buffer : t) : int array array = buffer.data
...
let init (dimensions : int * int) (f : int -> int -> int) : t =
  ...
  {
    data = Array.init height (fun y -> Array.init width (fun x -> f x y));
    dirty = true;
  }
...
let pixel_write (x : int) (y : int) (col : int) (buffer : t) =
  if
    x >= 0
    && x < Array.length buffer.data.(0)
    && y >= 0
    && y < Array.length buffer.data
  then buffer.data.(y).(x) <- col;
  buffer.dirty <- true
...
let filled_circle (x : int) (y : int) (r : float) (col : int) (buffer : t) =
  ...
  let pminy = fy -. r and pmaxy = fy +. r in
  ...
  for yi = Int.of_float miny to Int.of_float maxy do
    let a = acos (Float.of_int (yi - y) /. r) in
    let xw = sin a *. r in
    let pminx = fx -. xw and pmaxx = fx +. xw in
...
let draw_ellipse (x0 : int) (y0 : int) (a : float) (b : float) (col : int)
    (buffer : t) =
  let x = ref 0 in
...
let filled_ellipse (x : int) (y : int) (rx : float) (ry : float) (col : int)
    (buffer : t) =
  let fx = Float.of_int x and fy = Float.of_int y in
  let my = Float.of_int (Array.length buffer.data - 1)
  and mx = Float.of_int (Array.length buffer.data.(0) - 1) in

  let pminy = fy -. ry and pmaxy = fy +. ry in
  let miny = if pminy < 0. then 0. else pminy
  and maxy = if pmaxy > my then my else pmaxy in
...
let draw_line (x0 : int) (y0 : int) (x1 : int) (y1 : int) (col : int)
    (buffer : t) =
  let dx = abs (x1 - x0)
  ...
  let rec loop (x : int) (y : int) (error : int) =
    ...
    | false ->
        let e2 = 2 * error in
        let nx = match e2 >= dy with false -> x | true -> x + sx in
        let ny = match e2 <= dx with false -> y | true -> y + sy in
        let nex = match e2 >= dy with false -> 0 | true -> dy in
        let ney = match e2 <= dx with false -> 0 | true -> dx in
...
let draw_rect (x : int) (y : int) (width : int) (height : int) (col : int)
    (buffer : t) =
  draw_polygon
    [ (x, y); (x + width, y); (x + width, y + height); (x, y + height) ]
    col buffer

let filled_rect (x : int) (y : int) (width : int) (height : int) (col : int)
    (buffer : t) =
  for oy = 0 to height do
    draw_line x (y + oy) (x + width) (y + oy) col buffer
  done

let draw_triangle (x0 : int) (y0 : int) (x1 : int) (y1 : int) (x2 : int)
    (y2 : int) (col : int) (buffer : t) =
  draw_line x0 y0 x1 y1 col buffer;
  draw_line x1 y1 x2 y2 col buffer;
  draw_line x2 y2 x0 y0 col buffer

type span = Only of int | Pair of int * int
...
let interpolate_line (x0 : int) (y0 : int) (x1 : int) (y1 : int) : span array =
  ...
  let result : span option array =
    Array.init (abs (y1 - y0) + 1) (fun _i -> None)
  in
  ...
  Array.map
    (fun maybe_span ->
      match maybe_span with None -> assert false | Some span -> span)
    result

let leftmost span = match span with Only x -> x | Pair (x0, _) -> x0
let rightmost span = match span with Only x -> x | Pair (_, x1) -> x1

let filled_triangle (x0 : int) (y0 : int) (x1 : int) (y1 : int) (x2 : int)
    (y2 : int) (col : int) (buffer : t) =
  ...
  let other_edge =
    if y1 == y0 then interpolate_line x1 y1 x2 y2
    else if y1 == y2 then interpolate_line x0 y0 x1 y1
    else
    ...
    (fun i s ->
      ...
      if index >= 0 && index < Array.length buffer.data then
        ...
        if r1 > 0 && r0 < stride - 1 then
          ...
          and x1 =
            if r1 < 0 then 0 else if r1 >= stride then stride - 1 else r1
          in
          for x = x0 to x1 do
...
let poly_to_strands (points : (int * int) list) : strand list =
  ...
  | points ->
      ...
      let rec loop (last_direction : int) (current_strand : strand)
          (result : strand list) (remaining : ((int * int) * (int * int)) list)
          : strand list =
        match remaining with
        | [] -> ( match current_strand with [] -> result | x -> x :: result)
        | hd :: tl ->
            let p0, p1 = hd in
            let _, y0 = p0 and _, y1 = p1 in
            let diff = y1 - y0 in
            ...
            else loop direction [ hd ] (current_strand :: result) tl
      in
      ...
      if List.length unwrapped < 2 then unwrapped
        ...
        if head_dir != end_dir then unwrapped
          ...
          let fore = List.hd unwrapped and shortened = List.tl unwrapped in
...
let interpolate_strand (strand : ((int * int) * (int * int)) list) :
    int * span array =
  let spans =
    List.map
      (fun line ->
        ...
        if y0 <= y1 then (y0, interpolate_line x0 y0 x1 y1)
        else (y1, interpolate_line x1 y1 x0 y0))
      strand
  in
  let sorted_spans =
    List.sort
      (fun a b ->
        let y0, _ = a and y1, _ = b in
        y0 - y1)
      spans
  in
      ...
      Array.iteri
        (fun i row ->
          ...
          if index >= 0 && index < height then
            ...
            let sorted_row =
              List.sort (fun a b -> leftmost a - leftmost b) row
            in
            ...
            let rec loop pairs =
              ...
              | raw_x0 :: [] -> (
                  ...
                  | Only x ->
                      let x0 =
                        if x < 0 then 0
                        else if x >= stride then stride - 1
                        else x
                      in
                  ...
                  | Pair (raw_x0, raw_x1) ->
                      let x0 =
                        if raw_x0 < 0 then 0
                        else if raw_x0 >= stride then stride - 1
                        else raw_x0
                      and x1 =
                        if raw_x1 < 0 then 0
                        else if raw_x1 >= stride then stride - 1
                        else raw_x1
                      in
                      let dist = x1 - x0 + 1 in
                      if dist > 0 then Array.fill brow x0 dist col)
              | raw_x0 :: raw_x1 :: tl ->
                  let rx0 = leftmost raw_x0 and rx1 = rightmost raw_x1 in
                  (if rx1 > 0 && rx0 < stride - 1 then
                     let x0 =
                       if rx0 < 0 then 0
                       else if rx0 >= stride then stride - 1
                       else rx0
                     and x1 =
                       if rx1 < 0 then 0
                       else if rx1 >= stride then stride - 1
                       else rx1
                     in
                     let dist = x1 - x0 + 1 in
      ...
      for h = 0 to gh - 1 do
        for w = 0 to bytes_per_line - 1 do
          ...
          for bit = 0 to bitcount - 1 do
            ...
            match isbit with
            | 0 -> ()
            | _ -> pixel_write (x + (w * 8) + bit) (y + h) col buffer
          done
        done
      done;
      gw

let draw_string (x : int) (y : int) (f : Font.t) (s : string) (col : int)
    (buffer : t) =
...
let map (f : shader_func) (buffer : t) : t =
  { data = Array.map (fun row -> Array.map f row) buffer.data; dirty = true }

let mapi (f : shaderi_func) (buffer : t) : t =
  {
    data =
      Array.mapi
        (fun y row -> Array.mapi (fun x _p -> f x y buffer) row)
        buffer.data;
    dirty = true;
  }
...
let map_inplace (f : shader_func) (buffer : t) =
  Array.iter
    (fun row -> Array.iteri (fun i v -> row.(i) <- f v) row)
    buffer.data;
  buffer.dirty <- true

let mapi_inplace (f : shaderi_func) (buffer : t) =
  Array.iteri
    (fun y row -> Array.iteri (fun x _v -> row.(x) <- f x y buffer) row)
    buffer.data;
  buffer.dirty <- true
...
let render (buffer : t) (draw : Primitives.t list) =
  List.iter
    (fun prim ->
      ...
      | Primitives.Ellipse (point, a, b, col) ->
          draw_ellipse point.x point.y a b col buffer
      | Primitives.FilledEllipse (point, a, b, col) ->
          filled_ellipse point.x point.y a b col buffer
      ...
      | Primitives.Polygon (plist, col) ->
          draw_polygon
            (List.map (fun (p : Primitives.point) -> (p.x, p.y)) plist)
            col buffer
...
let map2 (f : int -> int -> int) (origin : t) (delta : t) : t =
  ...
  with Invalid_argument _ ->
    raise
      (Invalid_argument
         "Merging framebuffers requires both to have same dimensions")
...
let map2_inplace (f : int -> int -> int) (origin : t) (delta : t) =
  try
    Array.iter2
      (fun o_row d_row ->
        Array.iteri
          (fun index d_pixel -> o_row.(index) <- f o_row.(index) d_pixel)
          d_row)
      origin.data delta.data;
    origin.dirty <- true
  with Invalid_argument _ ->
...
let is_dirty (buffer : t) : bool = buffer.dirty
let set_dirty (buffer : t) : unit = buffer.dirty <- true
let clear_dirty (buffer : t) : unit = buffer.dirty <- false
