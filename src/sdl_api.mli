module type SDL_API = sig
  ...
  val init : int -> string -> bool -> (window * renderer, string) result
  (**This handle the initialization of SDL
    arguments width, height, window title and fullscreen flag and onsuccess, this returns a window and a renderer and on failure we have an error message
  *)

  val create_texture : renderer -> Sdl.Pixel.format -> width:int -> height:int -> Sdl.Texture.access -> (texture, string) result
  (* Create a texture, given a renderer, a pixel format and dimension of width and height if successful, a texture is returned else an error*)

  val update_texture : renderer -> BigArray.Array1.t -> int -> (unit, string) result
