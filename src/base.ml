...
(* Unified input event type: now both keyboard and mouse events are      *)
type event =
  ...
  | MouseMotion of { x: int; y: int; xrel: int; yrel: int } (* Mouse moved *)
  | MouseWheel of { x: int; y: int }                       (* Mouse wheel scrolled *)
...
(* poll_all_events: Recursively polls all pending SDL events, updating   *)
(* the keys set, the mouse state, and accumulating a unified event list  *)
(* that includes both keyboard and mouse events.                         *)
let rec poll_all_events keys mouse events =
  ...
  | true ->
      ...
      (match typ with
       | `Quit ->
           (true, keys, mouse, List.rev events)
       | `Key_down ->
           let key = PlatformKey.of_backend_keycode (Sdl.Event.(get e keyboard_keycode)) in
           poll_all_events (KeyCodeSet.add key keys) mouse (KeyDown key :: events)
       | `Key_up ->
           let key = PlatformKey.of_backend_keycode (Sdl.Event.(get e keyboard_keycode)) in
           poll_all_events (KeyCodeSet.remove key keys) mouse (KeyUp key :: events)
       | `Mouse_button_down ->
           let button = Sdl.Event.(get e mouse_button_button) in
           let new_mouse = PlatformMouse.handle_event e mouse in
           poll_all_events keys new_mouse (MouseButtonDown button :: events)
       | `Mouse_button_up ->
           let button = Sdl.Event.(get e mouse_button_button) in
           let new_mouse = PlatformMouse.handle_event e mouse in
           poll_all_events keys new_mouse (MouseButtonUp button :: events)
       | `Mouse_motion ->
           ...
           let new_mouse = PlatformMouse.handle_event e mouse in
           poll_all_events keys new_mouse (MouseMotion { x; y; xrel; yrel } :: events)
       | `Mouse_wheel ->
           let x = Sdl.Event.(get e mouse_wheel_x) in
           let y = Sdl.Event.(get e mouse_wheel_y) in
           let new_mouse = PlatformMouse.handle_event e mouse in
           poll_all_events keys new_mouse (MouseWheel { x; y } :: events)
       | _ ->
           poll_all_events keys mouse events)
  | false -> (false, keys, mouse, List.rev events)
    ...
    | Ok texture ->
      ...
      in
          ...
          let rec loop (t : int) (prev_buffer : Framebuffer.t) (input : input_state) last_t =
            ...
            in
            let current_input = { keys = new_keys; events = unified_events; mouse = new_mouse } in
