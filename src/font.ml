...
let load_glyphs (reader : reader_t) header : (bytes array, string) result =
  ...
  try
    Result.ok (Array.init (Int32.to_int header.number_of_glyphs) (fun i ->
      ...
      reader (Int64.of_int ((Int32.to_int header.headersize) + (i * bpg))) bpg buffer;
      buffer
    ))
  with
  | Sys_error(reason) -> Result.error reason
...
let inner_load_map_table reader header : ((Uchar.t * int) list, string) result =
  try
    let offset: int64 ref = ref (Int64.of_int (((Int32.to_int header.headersize) + ((Int32.to_int header.number_of_glyphs) * (Int32.to_int header.bytes_per_glyph))))) in
...
let load_font reader =
  ...
  load_map_table reader header >>= fun map ->
