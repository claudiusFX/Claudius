...
let test_empty_event _ =
  ...
  | Error (`Msg e) ->
      failwith
        (Printf.sprintf "failed to push event: %s %s" e (Sdl.get_error ()))
  | Ok suc ->
      ...
      assert_equal ~printer:string_of_bool ~msg:"No mouse buttons" false
        (Mouse.is_button_pressed updated_mouse Left);
      assert_equal ~printer:string_of_bool ~msg:"No mouse buttons" false
...
let () =
  ...
  match success with
  | Error (`Msg e) ->
      failwith (Printf.sprintf "failed to init SDL: %s %s" e (Sdl.get_error ()))
  | Ok _ ->
      ();
