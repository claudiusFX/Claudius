...
let test_handle_mouse_button_event _ =
  ...
  let (updated_mouse2, _) = Mousesdl.handle_mouse_button_event event updated_mouse in
...
let test_drag_for_all_buttons _ =
  ...
  List.iter (fun button ->
    ...
    Sdl.Event.set event Sdl.Event.mouse_button_state Sdl.pressed;
    let (mouse_after_press, _) = Mousesdl.handle_mouse_button_event event mouse in
    (* Drag motion *)
    Sdl.Event.set event Sdl.Event.mouse_motion_x 30;
    Sdl.Event.set event Sdl.Event.mouse_motion_y 40;
    Sdl.Event.set event Sdl.Event.typ Sdl.Event.mouse_motion;
    let (_mouse_after_drag, events) = Mousesdl.handle_event event mouse_after_press in
    (* Assert Drag exists *)
...
let test_motion_after_drag_release _ =
  ...
  List.iter (fun button ->
    ...
    (* Release the button *)
    Sdl.Event.set event Sdl.Event.mouse_button_state Sdl.released;
    let (mouse_after_release, _) = Mousesdl.handle_mouse_button_event event mouse_after_drag in
...
let test_multiple_drag_events _ =
  ...
  let (mouse_after_middle, _) = Mousesdl.handle_mouse_button_event event2 mouse_after_left in
  (* Assert both buttons are pressed *)
  assert_bool "Expected Left button to be pressed" (Mouse.is_button_pressed mouse_after_middle Mouse.Left);
  assert_bool "Expected Middle button to be pressed" (Mouse.is_button_pressed mouse_after_middle Mouse.Middle);
  (* Simulate a drag motion *)
  Sdl.Event.set event Sdl.Event.mouse_motion_x 30;
  Sdl.Event.set event Sdl.Event.mouse_motion_y 40;
  Sdl.Event.set event Sdl.Event.typ Sdl.Event.mouse_motion;
  let (_mouse_after_drag, events) = Mousesdl.handle_mouse_motion_event event mouse_after_middle in
  (* Check drag events for both Left and Middle buttons *)
  assert_bool "Expected Drag event for Left button" 
    (List.exists (function | Event.MouseDrag (Mouse.Left, (30, 40)) -> true | _ -> false) events);
  assert_bool "Expected Drag event for Middle button" 
    (List.exists (function | Event.MouseDrag (Mouse.Middle, (30, 40)) -> true | _ -> false) events)
