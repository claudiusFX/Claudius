...
let test_fail_invalid_scale _ =
  let palette = Palette.generate_mono_palette 2 in
  assert_raises (Invalid_argument "Invalid scale") (fun _ ->
      Screen.create 640 480 (-1) palette)

let test_fail_invalid_dimensions _ =
  let palette = Palette.generate_mono_palette 2 in
  assert_raises (Invalid_argument "Invalid height") (fun _ ->
      Screen.create 10 0 2 palette);
  assert_raises (Invalid_argument "Invalid width") (fun _ ->
      Screen.create 0 10 2 palette);
  assert_raises (Invalid_argument "Invalid height") (fun _ ->
      Screen.create 10 (-10) 2 palette);
  assert_raises (Invalid_argument "Invalid width") (fun _ ->
      Screen.create (-10) 10 2 palette)
...
let test_update_palette _ =
  ...
  assert_equal ~msg:"Dirty flag should be false after clearing" false
    (Screen.is_dirty screen);
  let new_palette = Palette.generate_plasma_palette 2 in
  Screen.update_palette screen new_palette;
  assert_equal ~msg:"Palette should be updated" new_palette
    (Screen.palette screen);
  assert_equal ~msg:"Dirty flag should be true after update" true
    (Screen.is_dirty screen);
  Screen.clear_dirty screen;
  assert_equal ~msg:"Dirty flag should be cleared" false
    (Screen.is_dirty screen)
